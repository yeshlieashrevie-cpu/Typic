...
// Shows/hides the pickup note and the rush-delivery row depending on
// whichever shipping method is currently selected.
...
// Rush delivery only makes sense with a courier, so while it's on we lock
// the selection to Lalamove and grey out J&T and Pickup.
function setRushDisabledState(isRushOn) {
  const jntCard = $('.ship-card[data-courier="jnt"]', dom.shipOptions);
  const jntRadio = $('input[value="jnt"]', dom.shipOptions);
  const pickupCard = $('.ship-card[data-courier="pickup"]', dom.shipOptions);
  const pickupRadio = $('input[value="pickup"]', dom.shipOptions);
  ...
  if (isRushOn) {
    ...
    [jntCard, pickupCard].forEach((card) => {
      if (!card) return;
...
function handleRushToggle() {
  setRushDisabledState(dom.rushToggle.checked);
  updateShippingVisibility();
  updatePricing();
}
...
function populateDoneScreen(payload) {
  ...
  const shippingLine = payload.shipping_method === 'pickup'
    ...
    : `${shippingLabel(payload.shipping_method)}${payload.shipping_rush ? ' · Rush' : ''} to ${payload.address.city}, ${payload.address.province}`;
